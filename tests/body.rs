...
fn content_length_limit() {
    ...
    let res = warp::test::request().header("content-length", "999").reply(&limit);
    assert_eq!(res.status(), 413, "over limit returns 413");

    let res = warp::test::request().header("content-length", "2").reply(&limit);
...
fn json() {
    ...
    let req = warp::test::request().header("content-type", "application/json").body("[3, 2, 1]");
...
fn json_rejects_bad_content_type() {
    ...
    let req = warp::test::request().header("content-type", "text/xml").body("[3, 2, 1]");

    let res = req.reply(&json);
    assert_eq!(res.status(), 415, "bad content-type should be 415 Unsupported Media Type");
}
...
fn form_rejects_bad_content_type() {
    ...
    let req = warp::test::request().header("content-type", "text/xml").body("foo=bar");
    let res = req.reply(&form);
    assert_eq!(res.status(), 415, "bad content-type should be 415 Unsupported Media Type");
}
...
fn stream() {
    ...
    let body = warp::test::request().body("foo=bar").filter(&stream).expect("filter() stream");

    let bufs = futures::future::lazy(move || body.collect()).wait().expect("lazy");
