...
fn byte_ranges() {
    ...
    let res = warp::test::request().header("range", "bytes=100-200").reply(&file);
    ...
    // bad range
    let res = warp::test::request().header("range", "bytes=100-10").reply(&file);
    ...
    // out of range
    let res = warp::test::request().header("range", "bytes=100-100000").reply(&file);
    assert_eq!(res.status(), 416);
    assert_eq!(res.headers()["content-range"], format!("bytes */{}", contents.len()));
    assert_eq!(res.headers().get("content-length"), None);
    assert_eq!(res.body(), "");
