...
fn fail() {
    ...
    warp::test::ws().handshake(route).expect_err("handshake non-websocket route should fail");
}
...
fn closed() {
    ...
    let route = warp::ws2().map(|ws: warp::ws::Ws2| {
        ws.on_upgrade(|websocket| websocket.close().map_err(|e| panic!("close error: {:?}", e)))
    });
