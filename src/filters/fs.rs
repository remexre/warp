...
pub fn dir(path: impl Into<PathBuf>) -> impl FilterClone<Extract = One<File>, Error = Rejection> {
    ...
    ::get2().and(path_from_tail(base)).and(conditionals()).and_then(file_reply)
}
...
fn path_from_tail(
    ...
    ::path::tail()
        ...
        .and_then(|buf: PathBuf| {
            ...
            future::poll_fn(move || {
                let is_dir =
                    try_ready!(tokio_threadpool::blocking(|| buf.as_ref().unwrap().is_dir()));
            ...
            .map_err(|blocking_err: tokio_threadpool::BlockingError| {
                error!("threadpool blocking error checking buf.is_dir(): {}", blocking_err,);
                ...
                bytes_range(range, len)
                    ...
                    .unwrap_or_else(|BadRange| {
                        ...
                        *resp.status_mut() = StatusCode::RANGE_NOT_SATISFIABLE;
                        resp.headers_mut().typed_insert(ContentRange::unsatisfied_bytes(len));
                        resp
                    })
            }
        };
