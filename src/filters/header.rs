...
pub fn header<T: FromStr + Send>(
    ...
    filter_fn_one(move |route| {
        ...
        route
            ...
            .and_then(|value| value.to_str().map_err(|_| reject::known(InvalidHeader(name))))
...
pub(crate) fn header2<T: Header + Send>() -> impl Filter<Extract = One<T>, Error = Rejection> + Copy
{
    filter_fn_one(move |route| {
        ...
        route.headers().typed_get().ok_or_else(|| reject::known(InvalidHeader(T::NAME.as_str())))
    })
}
...
pub fn exact(
    ...
    filter_fn(move |route| {
        ...
        route.headers().get(name).ok_or_else(|| reject::known(MissingHeader(name))).and_then(
            |val| {
                if val == value {
                    Ok(())
                } else {
