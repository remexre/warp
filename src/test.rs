...
impl RequestBuilder {
    ...
    {
        let name: HeaderName =
            HttpTryFrom::try_from(key).map_err(|_| ()).expect("invalid header name");
        ...
        F::Error: Reject + Send,
    {
        ...
        let mut fut = route::set(&route, move || f.filter())
            ...
            .and_then(|res| {
                ...
                body.concat2().map(|chunk| Response::from_parts(parts, chunk.into()))
            });
        ...
        thread::Builder::new()
            ...
            .spawn(move || {
                ...
                *req.uri_mut() = uri;
                ...
                let read =
                    rx.then(|result| Ok(result)).forward(rd_tx.sink_map_err(|_| ())).map(|_| ());
    ...
    pub fn recv(&mut self) -> Result<::filters::ws::Message, WsError> {
        self.rx
            ...
            .map(|unbounded_result| {
                unbounded_result.map(|result| result.map_err(WsError::new)).unwrap_or_else(|_| {
